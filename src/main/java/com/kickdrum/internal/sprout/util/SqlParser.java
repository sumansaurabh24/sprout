...
public class SqlParser {
	...
	private StateOperationWrapper getStateFromAlter(Alter alter) {
		...
		// construct state
		State state = new StateBuilder().setSchema(schema).setTable(table).setOperation(StateOperation.DROP)
				.createState();
		...
		for (AlterExpression expression : alterExpressions) {
			...
			if (StateOperation.ADD.name().equalsIgnoreCase(op) || StateOperation.MODIFY.name().equalsIgnoreCase(op)) {
			...
			Operation operation = new OperationBuilder().setOperation(op).setModifiedAt(Instant.now())
					.setAffectedColumn(column).setModifiedBy(modifier).createOperation();
